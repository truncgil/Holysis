...
<div class="text-center" style="line-height:100px;"><div class="goto btn btn-primary d-none">{{e2("Goto Card On Board")}}</div></div>
<script type="text/javascript">
$(".goto").on("click",function(){
	localStorage.setItem('goto',"j{{get('id')}}");
});
...
?>
@if(!isset($sap[get("id")])) 
	<div class="alert alert-info">{{get("id")}} {{e2("Not found")}}</div>
<script type="text/javascript">
...
<div class="col-md-6">
	...
	<script type="text/javascript">
	var total = ("{{$veri[map('Bedarfsmenge SD')]}}".replace(".",""));
...
if(secici.length<miktar) {
	alert("{{e2("Settlement could not be made because the calendar is full. Number of remaining fields: ")}}"+secici.length);
} else {
	secici.each(function(){
		console.log("gelen boyut="+miktar);
		if(k<=miktar) {
			$(this).html('<div class="job fill j'+id+'" draggable="true" ondragstart="drag(event)"  jid="'+id+'" style="'+style+'" title="'+qty+'/'+' {{e2("Part")}} '+k+'">'+html+'<div class="delete"></div></div>');
		}
		k++;
	});
}
}
					var style = $("#j{{$veri['ID']}}").attr("style");
					...
					var total = (dizi['{{map('Bedarfsmenge SD')}}'].replace(".",""));
					total = (total.replace(",","."));
					$(".modal-body #BedarfsmengeSD").val(total);
					$(".modal-body #qty").attr("max",total).prop("max",total);
					console.log(total);
				//	console.log(dizi);
					...
					$(".modal-footer .delete").on("click",function(){
						...
						if(confirm("{{e2("Are you sure delete this card?")}}")) {
							console.log(secici);
							$(secici).remove();
							$(drag_secici).html($(plan_secici).parent().html());
							$(plan_secici).remove();
					...
					$(".schedule").unbind(); 
					var workstation = dizi['{{map('Arbeitsplatz')}}'];
						//eğer HAND veya END ise workstation listesini göster
						if(dizi['{{map('MRP Contr.')}}']=="HF" || dizi['{{map('MRP Contr.')}}']=="END") {
