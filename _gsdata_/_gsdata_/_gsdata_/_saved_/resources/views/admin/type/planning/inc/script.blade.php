...
function loader(id) {
	...
	var url = '?ajax=planning-board-load&type={{get("w")}}';
	//console.log("load-id:",id);
	if(id!==undefined) {
		url = '?ajax=planning-board-load&type={{get("w")}}&id='+id;
	}
	...
	$.getJSON(url, function(jd) {
		...
		//	alert("ok");
			...
			//	//console.log($(".drag-data #j"+d.jid).length);
				...
				if($(".drag-data #j"+d.jid).length==0) {
					alert("{{e2("This card is not found will can be deleted")}}");
		...
		//	$(".popover").hide();
			$("#popover_content_wrapper").load(("?ajax=planning-tooltip&w={{get("w")}}&id="+$(this).attr("jid")));
...
function jobModalDetail(jid,title) {
	...
	var url = "?ajax=planning-detail&w={{get("w")}}&id="+jid;
	//console.log(url);
...
<script>
  $( function() {
	  ...
	  <?php } ?>
	  $(".plan-ajax").html("{{e2("Loading...")}}").load(url,function(){
